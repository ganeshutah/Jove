...
def classify_l_id_path(l_id_path, l_visited_id, P, acceptance, STKMAX):
    """Helper for run_pda
       ---
    ...
    #print("l_visited_id = ", l_visited_id)
    
    surv_pool  = list(map(survivor_id(l_visited_id, P, STKMAX=STKMAX), l_id_path))
    term_pool  = list(map(term_id(l_visited_id, P, STKMAX=STKMAX), l_id_path))
...
def h_run_pda(l_id_path, l_term_id_path, l_final_id_path, l_visited_id, 
    ...
    while (l_id_path != []):
        ...
        if nl_id_path0 == []:
            ...
            (l_surv, 
             l_term, 
             l_final) = classify_l_id_path(nl_id_path0, l_visited_id, pda, acceptance, STKMAX)
...
def survivor_id(l_visited_id, pda, STKMAX):
    """Helper for classify_l_id_path
       ---
       Classify l_visited_id using is_surv_id to tag 
       its entries 'surv' or 'not_surv'.
    """
    return (lambda id_path:
                (("surv", id_path)
                    if is_surv_id(id_path, l_visited_id, pda, STKMAX=STKMAX)
                    else ("not_surv", id_path)))

def term_id(l_visited_id, pda, STKMAX):
    """Helper for classify_l_id_path
       ---
       Classify l_visited_id using is_term_id to tag 
       its entries 'term' or 'not_term'.
    """
    return (lambda id_path:
                (("term", id_path)
                    if is_term_id(id_path, l_visited_id, pda, STKMAX=STKMAX)
                    else ("not_term", id_path)))

def final_id(l_visited_id, pda, acceptance):
    """Helper for classify_l_id_path
       ---
       Classify l_visited_id using is_final_id to tag 
...
def is_surv_id(id_path, l_visited_ids, pda, STKMAX):
    ...
    #print("--is_surv_id--")
    (id, path) = id_path
    S = subsumed(id, l_visited_ids, STKMAX=STKMAX)
    #print("not S = ", (not S))
    return (not S)
       
from functools import reduce

def subsumed(id, l_visited_ids, STKMAX):
    ...
    while (l_visited_ids != []):
        ...
        if ((q==q1) and
            ...
            l_visited_ids = l_visited_ids[1:]
    return False


def is_term_id(id_path, l_visited_ids, pda, STKMAX):
    """Helper for term_id
       ---
