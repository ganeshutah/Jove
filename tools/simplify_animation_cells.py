...
# Two quoting variants exist in the wild.  The notebooks generated by nbgen
# carry  display(HTML('<link rel="stylesheet" ...'))  while the older hand-written
# ones carry the same call with LITERAL backslash-escaped quotes,
#   display(HTML('<link rel=\\"stylesheet\\" ...'))
# so matching the exact inner markup misses 30 legacy cells.  Match the shape of
# the call instead: a lone display(HTML(...)) mentioning font-awesome.
FA_RE   = re.compile(r"^\s*display\(\s*HTML\(.*font-awesome.*\)\s*\)\s*;?\s*$")
